...
const sendNotification = catchAsyncError(async (req, res, next) => {
    const { messageData, receiver_id ,chat,names,sender_id} = req.body;
    ...
    const data = await Notification.findOne({ sender_id: sender_id, receiver_id: receiver_id, status: true })
    if (data) {
